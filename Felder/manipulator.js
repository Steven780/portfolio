...
osgGA.OrbitManipulator2 = function () {
    ...
    this.height = 0;
	this.idleMotionWhenRelease = 3.0;
	this.motionWhenRelease = this.idleMotionWhenRelease;
...
osgGA.OrbitManipulator2.prototype = {
        ...
        if (this.timeout === undefined) {
            ...
            window.setTimeout(function() {
                if (Math.abs(that.dx) + Math.abs(that.dy) === 0.0) {
					that.motionWhenRelease = this.idleMotionWhenRelease;
    ...
    mouseup: function(ev) {
        ...
        if (this.pushHit !== undefined) {
			...
			jQuery("#TweetContent").fadeOut(500, function() {
